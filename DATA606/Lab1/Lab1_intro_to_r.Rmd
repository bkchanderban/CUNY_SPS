...
ggplot(data = arbuthnot, aes(x = year, y = girls)) + geom_point() + geom_smooth(method = "lm",colour = "blue") + labs(title = "An increasing trend of baptized girls is observed over the years.")
```

**The plot above showcases that the number of girls being baptized over time, according the Arbuthnot dataset, increased as the years progressed. This increase is evident in the upward slope of the blue line, which represents a fitted linear model. The rise in numbers maybe due to factors such as improved record-keeping systems, a general growth in the population itself, or some other societal influences.**
...
```{r plot-boys-proportion-over-time}

ggplot(data = arbuthnot, aes(x = year, y = boy_ratio)) + geom_point() +labs(title = "Proportion of Boys Over Time") + theme(plot.title = element_text(hjust = 0.5))

```

**The plot above displays the proportion of boys baptized over time using the Arbuthnot dataset. In this, one notable observation is that the proportion never falls below 0.5 in any given year. This indicates that, throughout the observed period, more boys than girls were being baptized every year.**
...
```{r examine-present-data}
glimpse(present)
```

**Upon examining the 'present' dataframe, we observe that it comprises of 63 rows and 3 columns. The column/variable names are those of 'year', 'boys', and 'girls'.**

```{r unique-years}
unique(present$year)
```

**Looking at the unique years within the dataframe, we can view that the dataset spans from 1940 to 2002, with each row representing a distinct year.**

2.  How do these counts compare to Arbuthnot's? Are they of a similar magnitude?

```{r compare-magnitudes}
...
**In examining summary statistics from both Arbuthnot's data and the present dataset, it is clear that the number of births in the present data is vastly larger. Arbuthnot's records showcase that the annual boy birth counts ranged from 2,890 to 8,426, and girl counts from 2,722 to 7,779. In contrast, the present dataset shows boy births ranging from 1,211,684 to 2,186,274 and girl births from 1,148,715 to 2,082,052 per year. The evident difference between the magnitudes highlights the population growth and the difference in the depths of record keeping between the two examined time frames.**
...
ggplot(data = present, aes(x = year, y = (boys)/(boys + girls))) + geom_point() +labs(title = "Proportion of Boys Over Time") + theme(plot.title = element_text(hjust = 0.5))

```

**Yes, the plot does indeed support Arbuthnot's historical observation. In this, the proportion of boys born within the U.S. was viewed consistently remaining slightly above 0.5, translating to more boys being born each year than girls.**
...
```{r solve-highest-birth-year}
present %>%
  mutate(total = boys + girls)%>%
  arrange(desc(total))
```

**After computing and sorting the total number of births, we can observe that the highest number of total births occurred in 1961, with approximately 4,268,326 recorded births.**
