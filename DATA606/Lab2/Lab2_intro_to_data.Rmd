...
**Yes, the histograms differ based upon bin size. Larger bins group many
delay times together, which obscures variation and hides the finer
details of the distribution. Smaller bins provide more differentiation
along the x axis, revealing the clustering of short delays and the
right-skewed shape more clearly.**
...
**Upon creating the new dataframe with the requested conditions, we can
see that 68 flights meet the criteria of heading to SFO in the month of
February.**
...
```{r sfo-flights-arrival-delays-plot}
...
```{r sfo-flights-summary-stats}

summary(sfo_feb_flights$arr_delay)
IQR(sfo_feb_flights$arr_delay)
```

**The histogram created from the sfo_feb_flights dataframe depicts a
right-skewed distribution of arrival delays. Moreover, most flights
arrived earlier than scheduled, with a median arrival delay of -11
minutes. However, a few flights experienced very large delays (up to 196
minutes), creating a long right tail in the distribution. Because of
this skewness and the presence of outliers, the median (-11 minutes) and
the IQR (23.25 minutes) are more appropriate summary statistics than the
mean.**
...
```{r summary-custom-list-arr_delays}
...
**Upon running the above code, it is observed that both DL (Delta Air
Lines Inc.) and UA (United Air Lines Inc.) have the highest
interquartile ranges for arrival delays among the carriers (22).
Therefore, these carriers exhibit the most variability in arrival
delays.**
...
**Choosing the month with the lowest mean departure delay can be
misleading because the mean is sensitive to extreme values. A few
unusually or heavily delayed flights can significantly influence it. In
contrast, the median is somewhat resistant to outliers and better
reflects the typical departure delay. However, the mean captures the
overall magnitude of the delays, including the extremes. Therefore, the
median represents the typical experience, whilst the mean reflects the
overall average impact of delays.**
...
**Based on the on-time departure percentages shown above, I would choose
to fly out of LaGuardia (LGA) as 72.8% of its flights departed on time
(defined as a departure delay of less than five minutes).**
...
```{r create-scatterplot-speed-vs-distance}

ggplot(data = nycflights, aes(x = avg_speed, y = distance)) + geom_point() + geom_smooth(method = "lm")

```

**The generated scatterplot shows a positive association between the
variables of average speed and distance. Flights covering longer
distances tend to have higher average speeds, as reflected by the upward
...
```{r replicating-scatter-plot}

three_carriers <-nycflights %>%
  filter(carrier %in% c('AA','DL','UA'))

ggplot(data = three_carriers, aes(x = dep_delay, y = arr_delay, color = carrier)) + geom_point()

```

**Based on the replicated scatterplot, flights with departures delays of
roughly 30 minutes or less often still arrive on time (with arrival
delays being equal to or less than 0). Beyond this point, the arrival
delays tend to remain in the positives, indicating that notable
departure delays are not likely to be recovered in the course of the
flight.**
