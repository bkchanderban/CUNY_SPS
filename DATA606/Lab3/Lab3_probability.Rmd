...
**A streak length of one implies a shot was a hit (H) followed by a
miss. In this case, the representation for a streak length of one would
appear as: HM \| . In the case of a streak length of zero, this is
indicative of a single miss (often preceded by another miss itself, if
it is not the first recorded shot in the streak sequence). A streak
length of zero may be represented as: M \| .**
...
```{r count-kobe-streak-lengths}

kobe_streak_raw <- calc_streak(kobe_basket$shot)

kobe_streak <- if (is.data.frame(kobe_streak_raw)) {
  tibble(length = as.numeric(kobe_streak_raw$length))
} else {
  tibble(length = as.numeric(kobe_streak_raw))
}

```

```{r visualize-kobe-streaks}

ggplot(data = kobe_streak, aes(x = length)) + geom_bar()

```

**In observing Kobe's streak lengths from the 2009 NBA finals, we can
say that the distribution is right-skewed, with most streaks being short
(0 or 1) and fewer instances of longer streaks. Kobe's typical streak
length (that is, the one with the highest recorded frequency) was zero
(0), and the longest streak of successive made shots was that of four
(4).**
...
**In the simulation of 100 flips of an unfair coin, 28 flips came up
heads (and 72 came up tails).**
...
The number above is completely arbitrary. If you need inspiration, you
...
**To reflect a shooting percentage of 45%, the sample() function must
include a prob argument that assigns the probabilities of 0.45 to
producing an "H" and 0.55 to "M" (that is, prob = c(0.45, 0.55)).
Additionally, to simulate 133 shots, the size argument must be adjusted
for this number, and the results were subsequently stored in
sim_basket.**
...
```{r calculate-sim-streak-of-sim-basket}
...
```{r visualize-sim-basket-streaks}

sim_streak_raw <- calc_streak(sim_basket)

sim_streak <- if (is.data.frame(sim_streak_raw)) {
  tibble(length = as.numeric(sim_streak_raw$length))
} else {
  tibble(length = as.numeric(sim_streak_raw))
}
...
**The distribution of streak lengths for the simulated independent
shooter is right-skewed, similar to Kobe's observed streak lengths, with
most streaks being short (0 or 1). The typical streak length (that is,
the most frequent) is 0, and the player's longest streak of successive
made shots in 133 attempts is 7. In comparison, Kobe's longest streak in
the 2009 NBA finals dataset was 4.**
...
**If I were to rerun the simulation of the independent shooter a second
time, I would expect the streak distribution be somewhat similar, since
we are using the same 45% probability of a successful shot and the same
number of attempted shots (133). However, if the seed were kept the same
as in the current simulation, the results would be exactly identical. If
the seed were changed or left unset, then the exact bar heights (and the
longest observed streak) could change due to randomness in a new run of
...
kobe_streak_v2 <- kobe_streak %>% mutate (shooter = "Kobe")
sim_streak_v2 <- sim_streak %>% mutate (shooter = "Simulated/Independent")

both_streaks <- bind_rows(kobe_streak_v2, sim_streak_v2)

ggplot(both_streaks, aes(x = length)) + geom_bar() + facet_wrap(~ shooter, ncol = 1)

```

**In looking at the generated graph comparing the distributions of the
two shooters (Kobe and the simulated one), we can observe that both are
right-skewed with most streaks being short (especially within the 0 to 1
range) and longer streaks occurring much less frequently. However,
Kobe's distribution is more concentrated at these shorter streaks and
drops off swiftly, while the simulated shooter's distribution displays a
slightly longer tail, with more streaks of length 2 and even the
occasional longer streak (up to 7 in this particular simulation run).**

**Based on this specific comparison, there appears to be no clear
evidence that a "hot hand" model fits Kobe's shooting patterns here,
since he does not exhibit an unusually high frequency of longer streaks
relative to the independent 45% shooter. If anything, the simulated
shooter produced longer streaks more frequently.**
